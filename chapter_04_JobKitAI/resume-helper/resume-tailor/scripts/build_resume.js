...
const { Document, Packer, Paragraph, TextRun, AlignmentType, BorderStyle,
        LevelFormat, convertInchesToTwip, ExternalHyperlink, ImageRun } = require('docx');
const fs = require('fs');
...
// ---- header photo ------------------------------------------------------
// spec.photo = { "path": "...", "width": px, "height": px } — optional.
// Rendered right-aligned above the name using a right-tab stop.
const photoRun = () => {
  const p = spec.photo;
  if (!p || !p.path) return null;
  const w = p.width || 100, h = p.height || 132;
  const img = new ImageRun({ type: 'jpg', data: fs.readFileSync(p.path), transformation: { width: w, height: h } });
  return new Paragraph({ alignment: AlignmentType.RIGHT, spacing: { after: 120 },
    tabStops: [{ type: 'right', position: 8930 }],
    children: [new TextRun({ text: '', font: T.font }), img] });
};
...
// ---- assemble ----------------------------------------------------------
const kids = [];
if (photoRun()) kids.push(photoRun());
